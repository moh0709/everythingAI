...
export function App() {
  ...
  async function addTypedSourcePath() {
    ...
    if (!pathValue) return setError('Enter a folder path first.');
  ...
  async function executePreview(preview: PreviewRecord) {
    if (preview.preview_status !== 'ready') return setMessage(`Blocked: ${preview.blocked_reason || 'not executable'}`);
