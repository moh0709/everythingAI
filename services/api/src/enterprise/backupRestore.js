...
const SECRET_KEY_RE = /(password|secret|token|databaseurl|connection(string|url)?|accesskey|secretkey|credential|authorization|privatekey)/i;
...
function normalizeObject(object, scope) {
  ...
  if (checksumVerified && !checksumSha256) {
    throw new Error('verified object checksum requires checksum evidence');
  }
  return {
    objectId,
    storageKey,
...
function normalizePostgres(postgres) {
  ...
  if (checksumVerified && !checksumSha256) {
    throw new Error('verified PostgreSQL backup checksum requires checksum evidence');
  }
  return {
...
export async function validateEnterpriseRestoreCandidate({
  ...
  if (requireVerifiedChecksums) {
    ...
    if (core.objects.some((object) => !object.checksumVerified)) {
      return blocked('one or more object checksums are unverified');
    }
  }
  ...
  try {
    ...
    if (!postgresResult || postgresResult.ok !== true) {
      return failed('isolated PostgreSQL restore validation failed');
    }

    for (const object of core.objects) {
      const objectResult = await adapters.object({
        object,
        scope: core.scope,
        target,
        dryRun: true,
